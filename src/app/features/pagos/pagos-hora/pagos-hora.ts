...
@Component({
  ...
  templateUrl: './pagos-hora.html',
  styleUrl: './pagos-hora.scss',
})
export class PagosHora implements OnInit {
  loading = false;
  ...
  cargar(): void {
    ...
    this.pagoDiarioService.getResumenPagosHora().subscribe({
      next: (resumen) => {
        console.log('Resumen pagos HORA:', resumen);
        this.resumen = resumen;
        // Solo cargos de empleados (ya vienen filtrados por HORA del backend)
  ...
  get empleadosFiltrados(): EmpleadoPagoResumen[] {
    ...
    // Los empleados ya vienen filtrados por HORA del backend
  ...
  get totalHora(): number {
    return this.empleadosFiltrados.reduce((sum, e) => sum + e.totalACobrar, 0);
  }
  ...
  cerrarPeriodo(): void {
    ...
    this.reporteService.generarReporteHora(desde, hasta).subscribe({
      next: (reporte) => {
        this.snackBar.open('Reporte de horas generado correctamente', 'Cerrar', { duration: 2000 });
